...
> [!TIP]
> **10. 실시간 뉴스 AI 해석기 (Symmetrical News AI Interpreter)**
> * **역할**: 대시보드 우측 사이드바 뉴스 카드 선택 시 비동기로 기동하여, 해당 뉴스가 종목 주가 및 향후 전망에 미칠 영향(호재/악재)을 3~4줄의 마크다운 형식으로 해석해 줍니다. 메인 백테스트 그래프 연동과는 독립적으로 대칭 가동됩니다.
> * **물리 코드**: `backend/app/routers/market.py` 내 `interpret_news` 라우터 함수
> * **사용 도구**: `create_llm_client` (사용자가 프론트엔드 설정창에 설정한 로컬/클라우드 LLM 엔드포인트 정보 대칭 바인딩)
...
4. **Bloomberg식 글로벌 3단 레이아웃 및 UX 영속화**:
   * 대시보드 화면을 좌측(파라미터/이력), 중앙(차트/노드 파이프라인), 우측(실시간 뉴스 피드)의 3단 독립 스크롤 레이아웃으로 설계하여 투자 정보 인지 능력을 대폭 상향했습니다. 또한, `localStorage` 기반 상태 보존 정책을 적용하여 사용자 설정 및 API 인증값이 페이지 새로고침 시에도 영구 보존되도록 구현했습니다.
