...
## 📈 AI-Quant 백테스트 및 검증 엔진 고도화

최근 버전은 단순한 AI 투자 리포트 생성기를 넘어, AI thesis와 결정론적 가격 계산을 분리한 **백테스트 가능한 AI-Quant 분석 시스템**으로 확장되었습니다. 실제 주문 API, 계좌 연동, 체결 처리, 브로커 연동은 포함하지 않으며, 저장된 분석 신호를 정량적으로 검증하는 데 초점을 둡니다.

```mermaid
graph TD
    AI["AI Report Layer<br/>뉴스, 감성, 펀더멘털, 투자 thesis"] --> Signal["Structured TradeSignal<br/>raw_json.trade_signal"]
    OHLCV["OHLCV Data"] --> Quant["Deterministic Quant Layer<br/>entry, stop, target, RR, size"]
    Signal --> Gate["Signal Gate<br/>data quality, RR, volume, volatility"]
    Quant --> Gate
    Gate --> Backtest["BacktestEngine<br/>next-bar, stop/take, trailing, SELL exit"]
    Backtest --> Validation["ValidationEngine<br/>walk-forward, parameter sweep, risk flags"]
    Validation --> Dashboard["Backtest Dashboard<br/>validation console"]
```

### 핵심 개선 사항

- **결정론적 가격 계산 레이어**: `tradingagents/quant/price_evaluation.py`에서 실제 OHLCV DataFrame 기반으로 ATR, 추세 점수, 변동성 레짐, 거래량 점수, 지지/저항, 진입 구간, 손절가, 목표가, risk/reward, 포지션 비중을 계산합니다.
- **구조화 신호 우선 처리**: 백테스트는 자연어 리포트보다 `Decision.raw_json.trade_signal`에 저장된 structured signal을 우선 사용합니다. 자연어 파서는 legacy fallback으로 유지됩니다.
- **Signal Gate**: AI가 BUY 의견을 내더라도 손절/목표가, risk/reward, 가격 매력도, 데이터 품질, 거래량, 변동성 조건을 통과하지 못하면 HOLD, WAIT, WAIT_FOR_PULLBACK으로 낮출 수 있습니다.
- **백테스트 신뢰성 보정**: 신호일 당일 종가 진입을 피하고 다음 거래일에 진입하는 next-bar execution을 사용합니다. 가격 결측은 미래 가격으로 backfill하지 않고 forward-fill만 사용합니다.
- **현실적인 청산 로직**: long-only 기준으로 stop_loss, take_profit, trailing_stop_pct, SELL/UNDERWEIGHT long exit, 수수료/슬리피지를 반영합니다. 같은 일봉에서 stop과 target이 모두 닿으면 보수적으로 stop_loss를 우선합니다.
- **검증 고도화 API**: `/api/performance/walk-forward`, `/api/performance/parameter-sweep`, `/api/performance/validation-summary`를 통해 기간 편향, 파라미터 민감도, risk flag 집중도를 확인할 수 있습니다.
- **백테스트 종합 관리 UI**: Backtest Dashboard에 validation, walk-forward, parameter sweep, risk flags 탭이 추가되어 일반 백테스트와 검증 결과를 한 화면에서 관리할 수 있습니다.

### 주요 백엔드 API

| API | 용도 |
|---|---|
| `POST /api/performance/backtest` | 저장된 decision signal 기반 포트폴리오 백테스트 |
| `POST /api/performance/walk-forward` | train/test window 기반 walk-forward validation |
| `POST /api/performance/parameter-sweep` | sizing, slippage, risk/reward, 가격 매력도 기준별 parameter sweep |
| `POST /api/performance/validation-summary` | 일반 백테스트, sweep, walk-forward, risk flag 집계 종합 요약 |
| `GET /api/performance/decisions` | 저장된 의사결정 신호 조회 |
| `GET /api/market/{ticker}` | OHLCV 및 기술지표 조회 |
| `GET /api/market/{ticker}/news` | 종목 뉴스 feed 조회 |
| `POST /api/news/interpret` | 선택 뉴스에 대한 AI 해석 |

### 관련 문서

- `docs/CODEX_MASTER_PLAN.md`: 전체 AI-Quant 고도화 마스터 플랜
- `docs/09_codex_ai_quant_improvements.md`: Phase 1~3-B 개선 작업 결과
- `docs/10_backend_test_validation_report.md`: 백엔드 테스트 과정과 Go/Conditional Go 판단 근거
...
개발 변경 후에는 다음 명령을 기준으로 백엔드와 프론트엔드의 핵심 동작을 확인합니다.
...
- TestClient 기반 API 계약
- structured signal 우선 처리
- deterministic quant price evaluation
- next-bar execution 및 no-bfill
...
> **주의**: 위 테스트는 대부분 Mock 기반입니다. 실제 yfinance, 뉴스, LLM 서버, 장시간 SSE/background run 안정성은 별도 live smoke test로 확인해야 합니다.
